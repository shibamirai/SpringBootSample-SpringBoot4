# Spring 解体新書第 2 版の変更点(2025/11/28 時点)
...
#### H2データベースをサーバーモードで利用する

※2025/11/28現在、"'spring.h2.console.enabled' is an unknown porperty." という警告が出ており、H2データベースコンソールが表示できなくなっているため、下記に従ってH2データベースのサーバーモードを使用してください。
...
1. schema.sql で、テーブル作成前に既存のテーブルを削除するようにする。（インメモリモードで毎回リセットされる動きに合わせるため）

```sql
DROP TABLE IF EXISTS employee;  -- この行を追加
...
1. H2 Database を起動する
  WindowsのスタートメニューからH2コンソールを起動し、保存済み設定を「Generic H2 (Server)」に変更、JDBC URLを「jdbc:h2:tcp://localhost/~/testdb」として接続する
